...
public class ExamResultServiceImpl implements ExamResultService {
    ...
    public ExamResultResponse save(ExamResultRequest request) {
        ...
        var entity = examResultRepository
                .findByExam_IdAndStudentSubject_Id(request.examId(), request.studentSubjectId()).orElse(new ExamResult());
        ...
        return examResultMapper.toResponse(entity);
    }
    ...
    public List<ExamResultResponse> saveBulk(ExamResultBulkRequest request) {
        ...
        for (var item : request.students()) {
            ...
            var ss = studentSubjectRepository.findById(item.studentSubjectId())
                    .orElseThrow(() -> new ResourceNotFoundException("StudentSubject not found with id " + item.studentSubjectId()));

            validateScore(item.score(), exam.getTotalMarks());
            validateExamOwnership(exam, ss);

            var entity = examResultRepository
                    .findByExam_IdAndStudentSubject_Id(request.examId(), item.studentSubjectId()).orElse(new ExamResult());
            ...
            responses.add(examResultMapper.toResponse(entity));
        }
